...
/**
 * GET /api/reconciliation/decision-engine
 * 
 * Server-side Business Intelligence & Scaling Decision Engine API (Phase 4).
 * Generates explainable, deterministic SKU scaling and operational recommendations.
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await generateDecisionEngineRecommendations(accountId, filter);
    ...
    return NextResponse.json(
      {
        success: false,
        message: 'Failed to generate decision engine recommendations',
