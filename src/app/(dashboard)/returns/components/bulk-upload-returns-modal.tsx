...
export function BulkUploadReturnsModal({ isOpen, onClose, onSuccess }: BulkUploadReturnsModalProps) {
  ...
  const downloadSample = () => {
    ...
    a.href = url;
    a.download = 'returns_import_template.csv';
    a.click();
  }
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const res = await apiFetch('/api/returns/bulk-upload', {
        method: 'POST',
        body: formData,
      });
      ...
      if (data.inserted > 0) {
        ...
        toast({
          ...
          description: `Successfully processed ${data.inserted} returns and adjusted stock.`,
        })
      }
    } catch (error: any) {
      toast({
          ...
          <div className="space-y-4 py-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="p-4 rounded-xl border bg-emerald-50 dark:bg-emerald-950/20 text-emerald-700 dark:text-emerald-400">
                <p className="text-xs font-bold uppercase">Restocked</p>
