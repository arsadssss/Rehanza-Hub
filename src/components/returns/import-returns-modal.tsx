...
import { FileUp, AlertCircle, CheckCircle2, RefreshCw, Upload } from 'lucide-react';
...
interface ImportReturnsModalProps {
...
/**
 * Generic Import Modal for Marketplace Returns
 * Dynamically handles API routing and summary display.
 */
export function ImportReturnsModal({ platform, onSuccess, onClose }: ImportReturnsModalProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Endpoint naming convention: /api/returns/import/[platform]
      const res = await apiFetch(`/api/returns/import/${platform}`, {
        method: 'POST',
        body: formData
      });
      ...
      if (res.ok) {
        ...
        toast({ 
          ...
          description: `Successfully processed ${platform.toUpperCase()} return report.` 
        });
  ...
  if (result) {
    const importedCount = result.imported ?? result.orders_imported ?? 0;
    const duplicateCount = result.duplicates ?? result.duplicates_skipped ?? 0;
    const failedCount = result.failed ?? result.failed_rows ?? 0;
      ...
      <div className="space-y-6 py-4">
        <div className="grid grid-cols-3 gap-4">
          <div className="p-4 rounded-2xl bg-muted/50 border border-border/50 text-center">
            ...
            <p className="text-2xl font-black text-foreground">{result.total_rows || 0}</p>
          </div>
          <div className="p-4 rounded-2xl bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-100 dark:border-emerald-900/30 text-center">
            ...
            <p className="text-2xl font-black text-emerald-700 dark:text-emerald-400">{importedCount}</p>
          </div>
          <div className="p-4 rounded-2xl bg-blue-50 dark:bg-blue-950/20 border border-blue-100 dark:border-blue-900/30 text-center">
            ...
            <p className="text-2xl font-black text-blue-700 dark:text-blue-400">{duplicateCount}</p>
          </div>
        </div>
  ...
  return (
    <div className="space-y-6 py-4">
      ...
      >
        ...
        <div>
          <h3 className="text-lg font-bold font-headline">{file ? file.name : `Select ${platformName} Return Report`}</h3>
