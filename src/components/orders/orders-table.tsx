...
export function OrdersTable({ orders, loading, onUploadClick }: OrdersTableProps) {
    ...
    return (
      <Card className="border-0 shadow-xl rounded-[2rem] bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl">
        <CardContent className="flex flex-col items-center justify-center py-32 text-center">
          ...
          <h3 className="text-2xl font-black font-headline tracking-tight">No orders imported yet</h3>
          <p className="text-muted-foreground mt-2 max-w-xs">Start building your sales history by uploading reports from your marketplace platforms.</p>
    ...
    <Card className="border-0 shadow-xl rounded-[2rem] overflow-hidden bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl">
      ...
      <div className="p-6 border-t border-border/50 flex flex-col sm:flex-row items-center justify-between gap-4">
        <p className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground">
          Showing <span className="text-foreground">{(currentPage - 1) * itemsPerPage + 1}</span> to <span className="text-foreground">{Math.min(currentPage * itemsPerPage, orders.length)}</span> of <span className="text-foreground">{orders.length}</span> records
        </p>
